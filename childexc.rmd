...
# From this plot we can say the average resting pulse throughout the experimental years was under 100 and hence normal.
...
# From this plot we can say the average pulse just after the run from 2008-2013 was consistently over 135 which is very high but after 2013 the average pulse rate decreases and even getting less than 120 is 2016 which can be considered good as pulse 1 is taken just after completing the run.
...
# From this plot we can say the average pulse after 5 minutes of the run was consistently less than 116 and reaches its lowest average in 2018.

cleanPSD%>%
group_by(`measurement date`) %>%
summarise(pulse10Mean = mean(`pulse 10'`)) %>%
ggplot(aes(x= `measurement date`, y=pulse10Mean)) + geom_line()
# From this plot we can say the average pulse after 10  minutes of the run was nearly same to the pulse rate after 5 minutes as they both satayed around than 110-115. The lowest average for pulse10 was recorded in 2008.
