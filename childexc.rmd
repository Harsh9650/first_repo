...
Q(1) Looking for errors in variable age. Since observations are taken twice a year so no id can have more than two observations in a single year.
...
Q.(2) In this question we will discuss about the individuals who ran incorrect distance.
...
Q.(3) Analysis of the observations were running distance is given but running time and running speed is missing.
...
Q.(5)  Observe the average pattern of pulse, SBP and DBP observations with respect to time over the period of experimental years.
...
Q(6) Relation betweeen at rest obseravtions, at 1 min obseravtions, at 5 min obseravtions and at 10 min obseravtions.
...
Q.(7) Analysis for a specific age group. Selecting age bin = 10, becuase childrens in age 10 group ran 1000 and 2000 metres.
...
set.seed(123) # This will preserve the result for every single time and without this the datavalue split up will be different each time. 
...
### Residual vs Fitted : 1) constant variance. 2) outliers :82,525,441. 3) Points are well scattered and mean of residuals seems to be a constant and hence linearity is not violated.
...
prediction_error
### prediction error rate for training dataset is 5.36%.
...
# Train the model
model <- train(pulse10 ~gender+pulse0+pulse1+pulse5+sbp5+sbp10+dbp5+dbp10, data = MyData, na.action=na.omit, method = "lm",
...
```{r}
### Because of a very large dataset exhaustive approach can't be used on myData, forward and backward method on myData increases cp and bic value and hence interaction terms on full dataset were not required.so now we use only those predictors which are available in the model for interactions.

subsetdata <- MyData[,c(2,7:10,13:14,17:18)]

subsetmodel<- regsubsets(pulse10 ~.^2,data = subsetdata ,nvmax = 18, method = "exhaustive",really.big=TRUE)
bs3<-summary(subsetmodel)

max(bs3$rsq)
which.max(bs3$rsq)
plot(bs3$rsq,xlab="Number of Variables", ylab = "RSquare", type ="l")
points(15,bs3$rsq[15], col ="red", cex = 2, pch =20)
### According to rsq best model is with predictor 18 but after 15 predictors there is not much signifiant increase.

min(bs3$rss)
which.min(bs3$rss)
plot(bs3$rss,xlab="Number of Variables", ylab = "RSS", type ="l")
points(15,bs3$rss[15], col ="red", cex = 2, pch =20)
### According to rss best model is with predictor 18, but after 15 predictors there is not much signifiant decrease.

min(bs3$cp)
which.min(bs3$cp)
plot(bs3$cp,xlab="Number of Variables", ylab = "CP", type ="l")
points(11,bs3$cp[11], col ="red", cex = 2, pch =20)
### According to cp best model is with predictor 18. The model with 14 predictors have the lowest bias as cp nearly equals to P(13).

min(bs3$bic)
which.min(bs3$bic)
plot(bs3$bic,xlab="Number of Variables", ylab = "bic", type ="l")
points(11,bs3$bic[11], col ="red", cex = 2, pch =20)
### According to bic best model is with predictor 8.
### Since for bic and cp 11 predictors provide reasonable results we can choose 11 predictor model.
...
### since the model with the interaction terms increases cp and bic values and not much significant increase in rsq is observed and the model with interaction is also not nested so, the model with out interaction is better. 
```
